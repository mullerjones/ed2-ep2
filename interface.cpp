...
public:
    ...
    int insere(string palavra)
    {
        ...
        if(palavra.length() < parametro) return -1;
        if(tabela->devolve(palavra) == -1) return -1;
        
        int numArestasAntes = numArestas;
        tabela->insere(palavra);
        numPalavras++;
        int novoNum = 0;
        for (int i = 0; i < numPalavras; i++)
        {
...
bool Grafo::checaAresta(int i, int j)
{
    ...
    if (!aux) aux = checaTrocaLetras(a, b, a.length());
